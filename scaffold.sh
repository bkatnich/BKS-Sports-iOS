...
print(f"     (or clone BKS-Basketball-Client-iOS and strip basketball-specific content)")
print(f"  2. In DependencyContainer.swift: change SportConfiguration.basketball → .{slug}")
print(f"  3. In BKSBasketballApp.swift:    change .sportConfiguration(.basketball) → .{slug}")
print(f"  4. Fill in teamAbbreviationByID in SportConfiguration+{swift_name}.swift")
print(f"  5. Wire API endpoints in the four service files")
print(f"  6. Update bundle ID, app name, and Firebase config")
print(f"  7. Run xcodegen generate in App/")
PYEOF
